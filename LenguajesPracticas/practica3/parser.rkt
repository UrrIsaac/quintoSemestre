...
; El trabajo del parser es pasar de s-expression a WAE+.
; Como sabemos la gramática y las aridades de las operaciones tenemos que tenerlas :
...
(define (parse sexp)
  (cond
    ...
    [(list? sexp)
     (cond
       [(eq? (first sexp) 'with) (parse-with sexp with )]
       [(eq? (first sexp) 'with*) (parse-with sexp with* )]
...
(define (parse-with sexp constructor check-dup?)
  (if (and (= (length sexp) 3) (list? (second sexp)) (>= (length (second sexp)) 1))
      (let* ([bindings-sexp (second sexp)]
             ...
             [ids           (map binding-id bindings)])
        (if (and check-dup? (has-duplicates? ids))
            (error 'parse "Syntax Error: expresion mal formada en parse")
